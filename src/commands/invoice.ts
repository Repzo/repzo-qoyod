...
export const sync_invoice = async (commandEvent: CommandEvent) => {
  try {
    ...
    if (!(repzo_invoice.client_id as any).integration_meta?.qoyod_id)
      throw new Error(
    ...
    const qoyod_invoice_body: QoyodInvoice = {
      invoice: {
        contact_id: (repzo_invoice.client_id as any).integration_meta?.qoyod_id, // qoyod_client.integration_meta?.qoyod_id,
