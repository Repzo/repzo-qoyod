...
export const sync_measureunits = async (commandEvent) => {
    ...
    try {
        ...
        const unit_query = qoyod_units === null || qoyod_units === void 0 ? void 0 : qoyod_units.product_unit_types.map((unit) => `${nameSpace}_${unit.id}`); // ??
        ...
        for (let i = 0; i < qoyod_units.product_unit_types.length; i++) {
            ...
            const repzo_unit = repzo_units.data.find((r_unit) => { var _a; return ((_a = r_unit.integration_meta) === null || _a === void 0 ? void 0 : _a.id) == `${nameSpace}_${qoyod_unit.id}`; });
            const body = {
...
const get_qoyod_units = async (serviceEndPoint, serviceApiKey, query) => {
    try {
