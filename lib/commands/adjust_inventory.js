...
export const adjust_inventory = async (commandEvent) => {
    ...
    try {
        ...
        commandLog.addDetail(`${(_b = qoyod_products.products) === null || _b === void 0 ? void 0 : _b.length} Products in Qoyod`);
        const repzo_warehouses = await repzo.warehouse.find({
            per_page: 50000,
        });
        commandLog.addDetail(`${(_c = repzo_warehouses === null || repzo_warehouses === void 0 ? void 0 : repzo_warehouses.data) === null || _c === void 0 ? void 0 : _c.length} Warehouses in Repzo`);
        const repzo_variants = await repzo.variant.find({
            per_page: 50000,
        });
        commandLog.addDetail(`${(_d = repzo_variants === null || repzo_variants === void 0 ? void 0 : repzo_variants.data) === null || _d === void 0 ? void 0 : _d.length} Variants in Repzo`);
        const repzo_measureunits = await repzo.measureunit.find({
            per_page: 50000,
        });
        commandLog.addDetail(`${(_e = repzo_measureunits === null || repzo_measureunits === void 0 ? void 0 : repzo_measureunits.data) === null || _e === void 0 ? void 0 : _e.length} Measure units Warehouses in Repzo`);
        ...
        await commandLog
            .addDetail(`${Object.keys(qoyod_inventories).length} Inventories with products in Qoyod`)
            .commit();
        for (let key in qoyod_inventories) {
            ...
            if (!repzo_warehouse) {
                ...
                result.failed_msg.push(`Adjust Inventory Failed >> Warehouse with integration_meta.qoyod_id: ${qoyod_warehouse_id} was not found`);
                result.failed++;
                continue;
            }
            ...
            qoyod_inventory.forEach((qoyod_item) => {
                ...
                if (!repzo_variant) {
                    ...
                    result.failed_msg.push(`Adjust Inventory Failed >> Variant with integration_meta.qoyod_id: ${qoyod_item.id} was not found`);
                    result.failed++;
                    return;
                }
                ...
                if (!repzo_measureunit) {
                    ...
                    result.failed_msg.push(`Adjust Inventory Failed >> Measure Unit with integration_meta.qoyod_id: ${qoyod_item.unit_type} was not found`);
                    result.failed++;
                    return;
                }
